...
class GoogleMapPlotter(object):
    ...
    def heatmap(self, lats, lngs, threshold=10, radius=10, gradient=None, opacity=0.6, dissipating=True):
        ...
        self.heatmap_points_list.append((heatmap_points, settings))
    ...
    def write_heatmap(self, f):
        for v in self.heatmap_points_list:
            (heatmap_points, settings_string) = (v[0], v[1])
            self.write_heatmap_single(f, heatmap_points,settings_string)

    def write_heatmap_single(self, f, heatmap_points, settings_string):
